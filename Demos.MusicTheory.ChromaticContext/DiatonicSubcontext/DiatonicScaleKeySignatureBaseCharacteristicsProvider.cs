...
namespace Demos.MusicTheory.ChromaticContext.DiatonicSubcontext
{
    ...
    public sealed class DiatonicScaleKeySignatureBaseCharacteristicsProvider
    {
        ...
        {
            List<ChromaticNoteElementary> characteristics = GetFlatsAndSharps(key).ToList();
            ...
            {
                characteristics.Add(new ChromaticNoteElementary(quality, NotationSymbols.None));
            }
            return characteristics.ToArray();
        }

        private static List<ChromaticNoteElementary> GetFlatsAndSharps(KeySignatures key)
        {
            List<ChromaticNoteElementary> baseCharacteristics = new List<ChromaticNoteElementary>();
